    ...
    <div class="d-flex justify-content-between align-items-md-center flex-md-row flex-column mb-4">
        ...
        <div>
            <select id="users-trends" class="form-select">
                ...
                <option value="last_week" {{ request()->get('period') === 'last_week' ? 'selected' : '' }}>Since last
                    week
                </option>
                <option value="week" {{ request()->get('period') === 'week' ? 'selected' : '' }}>This month</option>
                <option value="last_month" {{ request()->get('period') === 'last_month' ? 'selected' : '' }}>Since last
                    month
                </option>
                <option value="quater_year" {{ request()->get('period') === 'quater_year' ? 'selected' : '' }}>This
                    quater year
                </option>
                <option value="half_year" {{ request()->get('period') === 'half_year' ? 'selected' : '' }}>This half
                    year
                </option>
                <option value="month" {{ request()->get('period') === 'month' ? 'selected' : '' }}>This year</option>
                <option value="last_year" {{ request()->get('period') === 'last_year' ? 'selected' : '' }}>Since last
                    year
                </option>
                <option value="year" {{ request()->get('period') === 'year' ? 'selected' : '' }}>Last 5 years</option>
                <option value="custom" {{ !in_array(request()->get('period'), ['last_week', 'day', 'week',
                    'quater_year', 'half_year', 'month', 'year', 'last_month', 'last_year']) &&
                    !is_null(request()->get('period')) ? 'selected' : '' }}>Custom
                </option>
            </select>
        </div>
    </div>
        ...
        <div class="col">
            <div class="card shadow-sm h-100">
                <div class="card-body">
                    ...
                    <p class="card-text fs-2 fw-bold mb-1">{{ $totalUsers['count'] }}</p>
                    <p class="card-text fs-6">{{ $totalUsers['variation']['type'] }} of {{
                        $totalUsers['variation']['by']
                        }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="col">
            <div class="card shadow-sm h-100">
                <div class="card-body">
                    ...
                    <p class="card-text fs-2 fw-bold mb-1">{{ $totalProducts['count'] }}</p>
                    <p class="card-text fs-6">{{ $totalProducts['variation']['type'] }} of {{
                        $totalProducts['variation']['by'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="col">
            <div class="card shadow-sm h-100">
                <div class="card-body">
                    ...
                    <p class="card-text fs-2 fw-bold mb-1">{{ $totalOrders['count'] }}</p>
                    <p class="card-text fs-6">{{ $totalOrders['variation']['type'] }} of {{
                        $totalOrders['variation']['by']
                        }}</p>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        drawProductsStatusChart({
        !!$productsStatusTrends
        !!
    })
